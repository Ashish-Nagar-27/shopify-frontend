...
export function useGoogleAuth({ setConnectingGoogle, setGoogleConnected }: UseGoogleAuthProps) {
    ...
    const handleAccountsSubmit = async (selectedAccounts: string[]) => {
        try {
            ...
            const response = await onboardingApi.connectGoogleAdsAccounts(customerIdParam, refreshToken);
