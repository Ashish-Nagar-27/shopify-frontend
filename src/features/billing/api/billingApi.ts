...
export const billingApi = {
    ...
    subscribe: async (planId: string): Promise<{ confirmationUrl: string }> => {
        const response = await api.post<{ confirmationUrl: string }>("/billing/subscribe", { planId });
        return response.data;
    },
    getStatus: async (shopId: string): Promise<{ subscription: Subscription | null }> => {
        const response = await api.get<{ subscription: Subscription | null }>(`/billing/status?shopId=${shopId}`);
        return response.data;
    },
    changePlan: async (newPlanId: string, shopId: string): Promise<{ confirmationUrl: string }> => {
        const response = await api.post<{ confirmationUrl: string }>("/billing/change-plan", { newPlanId, shopId });
        return response.data;
    },
    cancelSubscription: async (shopId: string): Promise<{ success: boolean }> => {
        const response = await api.post<{ success: boolean }>("/billing/cancel", { shopId });
        return response.data;
    }
};
